import simplifyPath from "../Douglas_Peuker";
...
function MainFileRecordParser (fileText, length, tolerance) {
    let position = 100;
    const record = [];
    ...
    const Polygon = () => {
        ...
        numTotalPoints += content.numPoints;
        content.shapeType = 5;
        content.parts = [];
        for(let i = 0; i < content.numParts; i++) {
            content.parts.push(fileText.getInt32(position + 44 + 4 * i, true));
        ...
        content.parts.forEach((part, idx) => {
            let partsOfPoints = [];
            if(idx === content.numParts - 1) {
                partsOfPoints = content.points.slice(part, -1);
                partsOfPoints = simplifyPath(partsOfPoints, tolerance);
                points = points.concat(partsOfPoints);
            }
            else {
                partsOfPoints = content.points.slice(part, content.parts[idx + 1] - 1);
                partsOfPoints = simplifyPath(partsOfPoints, tolerance);
                points = points.concat(partsOfPoints);
                newParts.push(points.length);
            }
        });
        content.parts = newParts;
        content.points = points;
        content.numPoints = content.points.length;
        return content;
    }
